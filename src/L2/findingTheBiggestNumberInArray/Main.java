...
public class Main {
  public static void main(String[] args) {
    ...
    for (int i = 0;
        i < randomNumbersArray.length;
        i++) { // Initialize the array with random numbers
    ...
    for (int i : randomNumbersArray) { // Traversing the array and finding max number
      if (i > maximalNumberInArray) maximalNumberInArray = i;
    }
    System.out.println(Arrays.toString(randomNumbersArray));
    System.out.println("Max number in array: " + maximalNumberInArray);
    System.out.println(
        "Max number in array found by sort-method: "
            + findingMaxNumberInArrayBySort(randomNumbersArray));
  }
  ...
  public static int findingMaxNumberInArrayBySort(int[] numbersArray) {
    int[] safeCopyOfArray =
        Arrays.copyOf(
            numbersArray,
            numbersArray.length); // Arrays.sort is mutate array, that's why create a copy
